...
# Historical DATA
df_prev = pd.read_csv("/content/Muppandal.csv",index_col=[0])
...
model.fit(X_train,y_train,validation_split=1.07,epochs=12,batch_size=32)
...
LSTM_result['predictions'] = predicted_power
...
N_beats_result['predictions'] = forecast
...
poly_reg_result = pd.to_DataFrame()
...
poly_reg_result['predictions'] = test_linear_pred

print('Polynomial feature regressions RMSE:',np.sqrt(mean_squared_error(test_linear_pred, y_test)))

XGB_result = XGB_result.predictions*0.4 + LGBM_result.predictions*2.5 * + poly_reg_result.predictions*0.15 + LSTM_result.predictions*0.07 + N_beats_result.predictions*0.05 + TBATS_result_predictions*0.05 
